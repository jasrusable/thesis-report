...
\chapter{Method}
	...
	\section{The 3D model}
		...
		\subsection{Renderings of the model}
			...
			Blender offers three types of render engines, Blender, Cycles, and Game renderers.
			
			Renders of the model will be created under various different render settings, as well as under the different model conditions (described above). The effect of altering the render settings will be a subject of investigation.
...
\chapter{Results}
	...
	\section{3D model}
		...
		\subsection{Renders}
			...
			For this investigation, the Cycles renderer was used as it offered superior results.
			
			Two quality settings where adjusted when creating renders.

	\section{Comparing the photograph to the model}
		In order to perform the comparison between the photograph and renders of the model, OpenCV was used extensively. The features2d module offers various feature detection algorithms (SIFT, SURF, ORB) as well as feature matching algorithms (FLANN, BruteForce). Another OpenCV module used in this investigation was the image processing module which offers a number of image enhancement/manipulation functions which have been made use of in this investigation.
